...
import Model.Sesion;
import Vistas.consultarUsuariosEmpleado;
...
public class Coordinador implements ActionListener {
	...
	public static void InicioSesion(JButton btnIniciarSesion, JTextField textFieldDNI,
		...
		ArrayList<Empleado> empleList = con.cargarEmpleado();
		ArrayList<Administrador> adminList = con.cargarAdmins();

		boolean enlista = true;

		for (int i = 0; i < empleList.size(); i++) {

			if (empleList.get(i).getDNI().equalsIgnoreCase(textFieldDNI.getText())
					&& empleList.get(i).getContrasena().equals(String.valueOf(passwordFieldContraseña.getPassword()))) {
				enlista = true;
				JOptionPane.showMessageDialog(btnIniciarSesion, "¡Inicio de sesión correcto!");
				consultarUsuariosEmpleado ventana = new consultarUsuariosEmpleado();
				ventana.setVisible(true);
			} else if (enlista == false) {

				JOptionPane.showMessageDialog(btnIniciarSesion, "Las credenciales introducidas no son correctas.");
				break;
			}
		...
		for (int i = 0; i < adminList.size(); i++) {
			...
			if (adminList.get(i).getDNI().equalsIgnoreCase(textFieldDNI.getText())
					&& adminList.get(i).getContrasena().equals(String.valueOf(passwordFieldContraseña.getPassword()))) {
				enlista = true;
				JOptionPane.showMessageDialog(btnIniciarSesion, "¡Inicio de sesión correcto!");
